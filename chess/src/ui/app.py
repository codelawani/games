...
from .screens import LoadPrevious, QuitGame, Welcome, PawnPromotion
from .messages import (
	NewGame, PieceMoved, PiecePromotion, PieceSelected, PieceDeselected,
	SwitchPlayer
)
...
class ChessApp(App):
	...
	def action_get_state(self):
		...
		self.is_checkmated,
		self.is_stalemated,
		self.valid_moves) = self.game.get_game_state()
		self.is_saved = False
		...
		if self.on_check:
			...
			self.call_after_refresh(lambda: self.query_one(f"#box-{get_loc(king_pos)}", Box).add_class("check"))
	...
	def on_mount(self):
		...
		for msg in (NewGame(self), PieceSelected(self, (1, 2)), PieceMoved(self), SwitchPlayer(self), PiecePromotion(self, 1)):
	...
	def on_piece_promotion(self, message: PiecePromotion):
		...
		if self.selected_piece:
			self.on_piece_deselected(PieceDeselected(self, self.selected_piece))
		self.on_switch_player(SwitchPlayer(self))
		self.call_after_refresh(self.on_piece_deselected, PieceDeselected(self, (0, 0)))
